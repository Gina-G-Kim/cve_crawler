...
class SpaceSecurityCrawler:
    ...
    def write_json(self, path: Path, records: Sequence[CVERecord], comment_header: bool = True) -> None:
        ...
        # Prepare a bullet-style header from the summary helper (for human readers only).
        bundle_summary = self._build_json_summary(records)
        json_text = json.dumps(bundle, indent=2, ensure_ascii=False)
        if comment_header:
            # Emit bullet-style comments above the JSON body. Use '//' prefix.
            header_lines = [f"// - {line}" for line in bundle_summary]
            header = "\n".join(header_lines) + "\n\n" if header_lines else ""
            path.write_text(header + json_text, encoding="utf-8")
        else:
